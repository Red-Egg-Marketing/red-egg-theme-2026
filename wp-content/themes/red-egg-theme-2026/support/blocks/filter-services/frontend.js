/**
 * Filter Services – Frontend Component
 *
 * Hydrates #FilterServicesRoot with data from
 * the red-egg/v2/services REST endpoint.
...
const apiUrl = '/red-egg/v2/cpt-services';

// Display config read from the root's data attributes in
// initFilterServices() (initial page size + server sort).
...
const FilterServices = () => {
    ...
    if ( loading ) {
        return (
            <div className="filter-services__loading">
    ...
    return (
        <Fragment>
            <div className="filter-services__filters">
                <ResourceFilters
            ...
            <div className={ 'filter-services__grid' + ( cfg.melt ? ' has-melt' : '' ) }>
                ...
                { resources.length === 0 && (
                    <div className="filter-services__empty">
                        <h3>No services match your filters. Try a different selection.</h3>
                    </div>
                ) }
            </div>
...
function initFilterCaseStudies() {
    const root = document.getElementById( 'FilterServicesRoot' );
