/**
 * Icon Card Block – Edit Component
 *
 * Icon (image or inline SVG) + InnerBlocks for title (heading) and body (paragraph).
 */
...
const template = [
    [ 'core/heading', { level: 3, placeholder: 'Card title…', className: 'icon-card__title' } ],
    [ 'core/paragraph', { placeholder: 'Card description…', className: 'icon-card__body' } ],
];
...
const renderIconPreview = ( { icon, iconAlt, svgMarkup } ) => {
    ...
    return (
        <div className="icon-card__icon icon-card__icon--placeholder"></div>
    );
};
...
const EditIconCard = ( { attributes, setAttributes } ) => {
    ...
    return (
        <Fragment>
            <InspectorControls>
                <PanelBody title={ __( 'Card Icon', 'red-egg' ) } initialOpen={ true }>
                    <MediaUpload
